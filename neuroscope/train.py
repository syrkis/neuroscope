...
# from neuroscope.model import loss_fn, init, apply


"""
# functions
def hyperparam_fn():  # TODO: perhaps have hyperparam ranges be in config.yaml
    return {"batch_size": 32, "n_steps": 10000, "dropout_rate": 0.5}
...
def train_folds(kfolds, hyperparams, seed=0):
    ...
    for idx, (train_loader, val_loader) in enumerate(kfolds):
        ...
        rng, key = jax.random.split(rng)
        fold_metrics, fold_params = train_loop(
            key, opt, train_loader, val_loader, plot_batch, hyperparams
        )
