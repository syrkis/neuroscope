...
def k_fold_fn(images, args, meta_data, img_files, k=5):
    ...
    for i in range(k):
        train_idxs, val_idxs = map(jnp.array, train_test_split(range(len(images)), test_size=0.2, random_state=i))
        train_loader = get_loader(images[train_idxs], args, meta_data, [img_files[idx] for idx in train_idxs])
        val_loader = get_loader(images[val_idxs], args, meta_data, [img_files[idx] for idx in val_idxs])
